//! Listens to an order book on testnet and prints it.
//!
//! Run with: cargo run --bin print_book
...
fn print_order_detailed(order: &BookOrder, current_block: u64) {
    let expiry_str = if order.expiry_block() == 0 {
        "never".to_string()
    } else if order.expiry_block() <= current_block {
        format!("{} (EXPIRED)", order.expiry_block())
    } else {
        format!(
            "{} (+{})",
            order.expiry_block(),
            order.expiry_block() - current_block
        )
    };
